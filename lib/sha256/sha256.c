// SHA-256 (FIPS 180-4). Public domain, after Brad Conte's reference
// implementation. Verified in test/test_dock against the NIST KATs and against
// the digests baked into dock-vectors.json.
...
static void transform(sha256_ctx *c, const uint8_t d[64]) {
...
void sha256_final(sha256_ctx *c, uint8_t out[32]) {
  ...
  for (i = 0; i < 8; ++i) c->data[63 - i] = (uint8_t)(c->bitlen >> (8 * i));
  transform(c, c->data);

  // Big-endian output, per the spec.
  for (i = 0; i < 4; ++i)
